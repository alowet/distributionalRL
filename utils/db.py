...
def create_connection(db_file, il=True, uri=False):
    ...
    try:
        ...
        # 30 second timeout to handle concurrency better
            ...
            conn = sqlite3.connect(db_file, detect_types=sqlite3.PARSE_DECLTYPES, timeout=30, isolation_level=il,
                                   uri=uri)
        return conn
    except Error as e:
        raise Exception(e)
...
def select_db(db, table, select_str, where_str, where_vals, keys=True, unique=True):
    ...
    if unique:
        if len(ret) != 1:
            raise Exception(
                'Multiple entries (or zero entries) found with where string={}, vals={}. Please manually select entry'.format(
...
def analyze_neural(data_dir, table):
    ...
    foldernames_tosave, filenames_tosave = get_names_tosave(mouse_name, file_date, file_date_id,
    ...
    sessions = select_db(paths['db'], 'session', 'protocol', 'name=? AND exp_date=? AND has_' + table + '=1',
                         (mouse_name, file_date), unique=False)
    ...
    behavior_filename = copy_behavior(behavior_filepath, paths['behavior_fig_roots'][0], foldernames_tosave, mouse_name,
                                      file_date)
...
def get_session(beh_data_folder, day, time, table=None):
    ...
    if len(datafile_names) == 1:
        ...
        if table == 'imaging':
            ...
            which_session = np.argmin(np.abs(
                [dt - datetime.combine(date, datetime.strptime(file_time, '%H%M%S').time()) for file_time in
                 file_times]))
